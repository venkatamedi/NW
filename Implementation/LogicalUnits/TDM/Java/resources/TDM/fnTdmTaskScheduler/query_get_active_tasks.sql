...
SELECT task_id AS max_task_id,lu_id AS max_lu_id,max(task_execution_id) AS max_task_execution_id FROM @TDMDB_SCHEMA@.TASK_EXECUTION_LIST GROUP BY task_id,lu_id
)
SELECT  task_id,
        scheduler,
        be_id,
